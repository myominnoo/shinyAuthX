...
#' ids to a database.  Use these if you don't want to allow automatic login with cookies.
#'
...
js_cookie_to_r_code <- function(id, expire_days = 7) {
	glue::glue(.open = "{{{", .close = "}}}", '
	  shinyjs.getcookie = function(params) {
	    var cookie = Cookies.get("shinyAuthX");
	  ...
	  shinyjs.setcookie = function(params) {
	    Cookies.set("shinyAuthX", escape(params), { expires: {{{expire_days}}} });
	    Shiny.setInputValue("{{{id}}}", params);
	  }
	  shinyjs.rmcookie = function(params) {
	    Cookies.remove("shinyAuthX");
...
#' to make the shinyAuthX package more self-contained
#' @noRd
jscookie_script <- function() {
	shiny::includeScript(system.file("js-cookie/js-cookie.js", package = "shinyAuthX"))
}
